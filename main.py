...
class Editor:
    def __init__(self, master):
        self.master = master
        customtkinter.set_appearance_mode(theme_get)
        self.master.geometry("1250x700")
        self.master.iconbitmap("icon.ico")
        self.master.title("Code space")
        self.frame = Frame(self.master)
        self.frame.pack()

        self.lineNumber = customtkinter.CTkCanvas(
        ...
        filemenu.add_command(label="Новий", command=self.new_file_mo, accelerator="Ctrl + N")
        filemenu.add_command(
            label="Відкрити", command=self.open_file, accelerator="Ctrl + O"
        )
        filemenu.add_command(label="Відкрити з дирекорії", command=self.open_in_dir)
        filemenu.add_command(
            label="Зберегти", command=self.save_file, accelerator="Ctrl + S"
        )
        filemenu.add_command(
            label="Зберігти як...", command=self.save_as
        )
        filemenu.add_command(
            label="Закрити файл", command=self.close_tab, accelerator="Ctrl + W"
        )
        filemenu.add_command(label="Сумарна статистика", command=self.summary)
        filemenu.add_command(label="Вихід", command=self.exit, accelerator="Alt + F4")

        editmenu = Menu(
            menubar, tearoff=0, background="#2c2f33", foreground="light blue"
        )
        editmenu.add_command(label="Відмінити", command=self.undo, accelerator="Ctrl + Z")
        editmenu.add_command(label="Вернути", command=self.cmd_redo, accelerator="Ctrl + Y")
        editmenu.add_separator()
        editmenu.add_command(label="Вирізати", command=self.cut, accelerator="Ctrl + X")
        editmenu.add_command(label="Скопіювати", command=self.copy, accelerator="Ctrl + C")
        editmenu.add_command(label="Вставити", command=self.paste, accelerator="Ctrl + V")
        editmenu.add_command(label="Видалити", command=self.delete, accelerator="Del")
        editmenu.add_command(
            label="Вибрати все", command=self.select_all, accelerator="Ctrl + A"
        )
        editmenu.add_separator()
        editmenu.add_command(
            label="Конвертувати вибране у ВЕРХНІЙ регістр", command=self.to_Upper)
        editmenu.add_command(
            label="Конвертувати вибране у нижній регістр", command=self.to_Lower)
        
        editmenu.add_separator()
        editmenu.add_command(label="Дублювати рядок", command=self.duplicate_line)
        editmenu.add_command(label="Перейти на рядок", command=self.goto)
        editmenu.add_separator()
        editmenu.add_command(label="Знайти і замінити", command=self.find_replace, accelerator="Ctrl + F")
        editmenu.add_separator()
        ...
        windowmenu.add_command(label="Завжди на першому плані", command=self.alwaysontop)
        ...
        transparency.add_command(label="Скинути", command=self.resetTransparent)
        windowmenu.add_cascade(label="Прозорість", menu=transparency)
        ...
        menubar.add_cascade(label="Редагування", menu=editmenu)
        menubar.add_cascade(label="Вікно", menu=windowmenu)
        ...
        self.right_click_menu.add_command(
            label="Копіювати", command=self.copy, accelerator="Ctrl + C"
        )
        self.right_click_menu.add_command(
            label="Вставити", command=self.paste, accelerator="Ctrl + V"
        )
        self.right_click_menu.add_command(
            label="Видалити", command=self.delete, accelerator="Del"
        )
        self.right_click_menu.add_command(
            label="Вибрати все", command=self.select_all, accelerator="Ctrl + A"
        )
        ...
        self.calc_menu.add_command(label="Порахувати", command=self.calculate)
        self.calc_menu.add_command(label="Порахувати і вивести результат", command=self.calc_enter_res)

        self.speakmenu.add_command(label="Цілий файл       ", command=self.full_speak)
        self.speakmenu.add_command(label="Тільки виділене", command=self.right_speak)

        self.right_click_menu.add_cascade(label="Прочитати вголос", menu=self.speakmenu)
        ...
        self.highlightmenu.add_command(label="Виділити", command=self.highlight_note)
        self.highlightmenu.add_command(
            label="Прибрати виділення", command=self.clear_highlight
        )

        self.right_click_menu.add_cascade(label="Виділення", menu=self.highlightmenu)
        self.right_click_menu.add_cascade(label="Знайти у", menu=self.searchmenu)

        self.tab_right_click_menu = Menu(self.master, tearoff=0)
        self.tab_right_click_menu.add_command(label="Закрити вкладку", command=self.close_tab)
        ...
        self.nb.add(first_tab, text="Чернетка")
    ...
    def create_text_widget(self, frame):
        ...
        yscrollbar = customtkinter.CTkScrollbar(frame, command=textbox.yview)
        ...
        xscrollbar = customtkinter.CTkScrollbar(
    ...
    def finder(self, e):
        finde = customtkinter.CTkToplevel()
        finde.title("Знайти")
        textfindedd = customtkinter.CTkEntry(
            finde,
            width=200,
    ...
    def new_file(self, e):
        ...
        file_ui.title("Новий"), file_ui.geometry("250x150")
        ...
        type_selection = customtkinter.CTkComboBox(master=file_ui,
                                 values=["Python файл", "Текстовий файл"])
    ...
    def save_changes(self):
        ...
        if (
            ...
            m = messagebox.askyesnocancel(
                "Сode space",
                "Увага! Ви хочете зберегти файли перед закриттям",
            )
            if m is None:
                return False
            elif m is True:
